...
A. kernel_assets 制作方式不同
   ...
   - 必须将 .cuh 源码内联到 kernel.py 中（因为 KernelBench 不支持外部头文件路径）
   - reference.py 中的 Model 使用对应的 Python 接口（调用原始 jit_kernels 函数）作为正确性基准

B. kernel.py 模板（CUDA C）
   ...
   cuda_source = """
   // 在这里内联 .cuh 的完整 CUDA C 源码
   // 注意：不能 #include 外部头文件，必须自包含
   """

   cpp_source = """
   torch::Tensor kernel_func(torch::Tensor input, ...);
   """
...
E. 选择 kernel 的注意事项
   - jit_kernels/ 中每个 .py 文件都通过 call_jit 调用对应的 .cuh
   - .cuh 头文件在 $MMQ_KERNELS/include/<subdir>/ 下
   - 优先选择依赖少、自包含的 kernel（如 add、cumsum、embedding）
   - 跳过依赖 kittens 或复杂模板库的 kernel（内联成本太高）
   - 跳过 nvshmem 相关的 kernel（需要多节点环境）
   - 跳过 Blackwell 专用 kernel（需要 sm100 架构）

具体要求：

1. 选择 kernel
   - 通过 workspace/runs/ 和 history.db 确认哪些 kernel 已有优化分支，跳过它们
   - 检查目标 .cuh 的 #include 依赖，确认能够自包含内联
   - 确认每个 kernel 有配套的 kernel_assets（kernel.py + reference.py），没有的话先制作
...
⚠️ CUDA C 与 Triton 的关键区别（必须遵守）：

A. kernel.py 使用 torch.utils.cpp_extension.load_inline 编译内联 CUDA C 源码，不是 import Python 函数
B. .cuh 头文件的完整源码必须内联到 kernel.py 中（KernelBench 不支持外部头文件路径）
C. bench.sh 使用 --backend cuda
D. commit body 中 backend 字段写 cuda
E. 跳过依赖 kittens/nvshmem/Blackwell 的 kernel
F. 检查 .cuh 的 #include 依赖链，只选可自包含内联的 kernel
...
- 连续 {{MAX_SKIP}} 个 kernel 因依赖复杂（如 kittens 模板库、外部头文件链）被跳过
- 我主动说"停"

每轮开始前：
1. 查询 history.db 获取已优化 kernel 列表
2. 扫描 $MMQ_KERNELS/jit_kernels/ 获取全部 CUDA C kernel（使用 call_jit 的 .py 文件）
3. 对每个候选 kernel，检查 $MMQ_KERNELS/include/ 下的 .cuh 头文件依赖
4. 差集中排除依赖过重的 kernel，剩余即为待优化列表
5. 从待优化列表中选 {{BATCH}} 个
...
1. 选择 kernel
   - 通过 workspace/runs/ 和 history.db 确认哪些 kernel 已有优化分支，跳过它们
   - 检查目标 .cuh 的 #include 依赖，确认能够自包含内联
   - 优先选择依赖少、自包含的 kernel（如 cumsum、embedding、permute）
   - 跳过依赖 kittens 或复杂模板库的 kernel（内联成本太高）
   - 确认每个 kernel 有配套的 kernel_assets（kernel.py + reference.py），没有的话先制作
...
- CUDA C kernel：从 jit_kernels/ 选择，kernel.py 用 load_inline 内联 .cuh 源码，bench 用 --backend cuda
- 每个 kernel 的 commit body 中 backend 字段必须准确填写（triton 或 cuda）
- CUDA C kernel 跳过依赖 kittens/nvshmem/Blackwell 的
- 优先选简单、自包含的 kernel
...
| 维度 | Triton | CUDA C |
|---|---|---|
| 源码位置 | `triton_kernels/*.py` | `jit_kernels/*.py` + `include/**/*.cuh` |
| kernel.py 加载方式 | `import` Python 函数 | `torch.utils.cpp_extension.load_inline` |
| 外部依赖 | 无（纯 Python/Triton） | 可能依赖 kittens、nvshmem 等 C++ 库 |
| bench.sh `--backend` | `triton` | `cuda` |
| commit body `backend` | `triton` | `cuda` |
| 编译速度 | 快（JIT） | 慢（nvcc 编译） |
| 优化手段 | block_size/warps/stages/eviction | blockDim/smem/vectorize/unroll/async |
| 建议并行数 | 3 | 2（编译慢，显存占用大） |
