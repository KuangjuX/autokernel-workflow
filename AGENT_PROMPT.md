...
B. kernel.py 制作策略（核心原则：复制源码到 cuda_source，让 agent 可以修改算法）
   ...
   模板：直接复制 .cuh 全部源码到 cuda_source，加上 C wrapper。

   ■ 策略二：依赖 kittens 的 kernel — 复制目标 .cuh 源码 + extra_include_paths 提供依赖

   核心思路：把目标 .cuh 的源码**复制**到 cuda_source 中（agent 可以自由修改算法），
   而它依赖的上游头文件（kittens.cuh、topk_scheduler.cuh 等）仍通过
   extra_include_paths + #include 引用，不需要复制。
   ...
   不能改的是：
   - kittens 库本身
   - 上游工具类（除非也一并复制到 cuda_source 中）

   前提条件：ThunderKittens submodule 必须已初始化。
   ...
   torch::Tensor select_topk_grad_cuda(
       ...
       auto scores_grad = torch::zeros({n_rows, FeaDim},
                                        topk_values_grad.options());
       mmq::kernels::router::select_topk_grad_optimized<FeaDim, K, NWarps>
           <<<num_sms, NWarps * 32>>>(
               topk_values_grad.data_ptr<float>(),
               topk_indices.data_ptr<int64_t>(),
               scores_grad.data_ptr<float>(), n_rows);
       return scores_grad;
   }
   """
   ...
   - 上游依赖（kittens.cuh 等）通过 #include 引用，不复制
...
- 查 PTX 指令时搜索 third_party/ptx-isa-markdown/cuda_skill/references/ptx-docs/
- 查 ncu 指标时搜索 third_party/ptx-isa-markdown/cuda_skill/references/ncu-guide.md
...
|---|---|---|---|---|
| 自包含 | topk_to_multihot | topk_to_multihot/topk_to_multihot.cuh | 仅 `<climits>` | 复制源码（纯内联） |
| 自包含 | cumsum | cumsum/cumsum.cuh | 无 | 复制源码（纯内联） |
